 ...
 * @param {Number} [east]
 * @param {Number} [north]
 * @param {Number} [timesRetried=0]
 * @param {Error} [error]
 */
function ModelMaskProviderError(
  provider,
  message,
  west,
  south,
 ...
 *
 * @param {ModelMaskProviderError} previousError The error instance returned by this function the last
 ...
 * @returns {ModelMaskProviderError} The error instance that was passed to the event listeners and that
 *          should be passed to this function the next time it is called for the same error in order
 *          to track retry counts.
 */
ModelMaskProviderError.reportError = function (
  previousError,
  provider,
  event,
  message,
  ...
  if (!defined(previousError)) {
    error = new ModelMaskProviderError(
      provider,
      message,
      west,
      south,
 ...
 *
 * @param {ModelMaskProviderError} previousError The previous error, or undefined if this operation has
 *        not previously resulted in an error.
 */
ModelMaskProviderError.reportSuccess = function (previousError) {
  if (defined(previousError)) {
 ...
 * @callback ModelMaskProviderError.RetryFunction
 */
export default ModelMaskProviderError;
