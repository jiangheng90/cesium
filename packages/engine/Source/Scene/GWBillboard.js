...
const SHOW_INDEX = (GWBillboard.SHOW_INDEX = 0);
const POSITION_INDEX = (GWBillboard.POSITION_INDEX = 1);
const PIXEL_OFFSET_INDEX = (GWBillboard.PIXEL_OFFSET_INDEX = 2);
const EYE_OFFSET_INDEX = (GWBillboard.EYE_OFFSET_INDEX = 3);
const HORIZONTAL_ORIGIN_INDEX = (GWBillboard.HORIZONTAL_ORIGIN_INDEX = 4);
const VERTICAL_ORIGIN_INDEX = (GWBillboard.VERTICAL_ORIGIN_INDEX = 5);
const SCALE_INDEX = (GWBillboard.SCALE_INDEX = 6);
const IMAGE_INDEX_INDEX = (GWBillboard.IMAGE_INDEX_INDEX = 7);
const COLOR_INDEX = (GWBillboard.COLOR_INDEX = 8);
const ROTATION_INDEX = (GWBillboard.ROTATION_INDEX = 9);
const ALIGNED_AXIS_INDEX = (GWBillboard.ALIGNED_AXIS_INDEX = 10);
const SCALE_BY_DISTANCE_INDEX = (GWBillboard.SCALE_BY_DISTANCE_INDEX = 11);
const TRANSLUCENCY_BY_DISTANCE_INDEX = (GWBillboard.TRANSLUCENCY_BY_DISTANCE_INDEX = 12);
const PIXEL_OFFSET_SCALE_BY_DISTANCE_INDEX = (GWBillboard.PIXEL_OFFSET_SCALE_BY_DISTANCE_INDEX = 13);
const DISTANCE_DISPLAY_CONDITION = (GWBillboard.DISTANCE_DISPLAY_CONDITION = 14);
const DISABLE_DEPTH_DISTANCE = (GWBillboard.DISABLE_DEPTH_DISTANCE = 15);
GWBillboard.TEXTURE_COORDINATE_BOUNDS = 16;
const SDF_INDEX = (GWBillboard.SDF_INDEX = 17);
GWBillboard.NUMBER_OF_PROPERTIES = 18;
...
GWBillboard.prototype.getPickId = function (context) {
  if (!defined(this._pickId)) {
...
GWBillboard.prototype._updateClamping = function () {
  GWBillboard._updateClamping(this._billboardCollection, this);
};

const scratchCartographic = new Cartographic();
const scratchPosition = new Cartesian3();

GWBillboard._updateClamping = function (collection, owner) {
...
GWBillboard.prototype._loadImage = function () {
 ...
 * </p>
 * <p>
 * To load an image from a URL, setting the {@link GWBillboard#image} property is more convenient.
 * </p>
 *
 * @param {String} id The id of the image.  This can be any string that uniquely identifies the image.
 * @param {HTMLImageElement|HTMLCanvasElement|String|Resource|GWBillboard.CreateImageCallback} image The image to load.  This parameter
 ...
 */
GWBillboard.prototype.setImage = function (id, image) {
  //>>includeStart('debug', pragmas.debug);
  if (!defined(id)) {
 ...
 */
GWBillboard.prototype.setImageSubRegion = function (id, subRegion) {
  //>>includeStart('debug', pragmas.debug);
  if (!defined(id)) {
    throw new DeveloperError("id is required.");
  }
...
GWBillboard.prototype._setTranslate = function (value) {
  //>>includeStart('debug', pragmas.debug);
  if (!defined(value)) {
    throw new DeveloperError("value is required.");
  }
...
GWBillboard.prototype._getActualPosition = function () {
  return defined(this._clampedPosition)
    ? this._clampedPosition
    : this._actualPosition;
};

GWBillboard.prototype._setActualPosition = function (value) {
...
GWBillboard._computeActualPosition = function (
  billboard,
  position,
  frameState,
  modelMatrix
...
GWBillboard._computeScreenSpacePosition = function (
  modelMatrix,
  position,
  eyeOffset,
  pixelOffset,
 ...
 *
 * @see GWBillboard#eyeOffset
 * @see GWBillboard#pixelOffset
 */
GWBillboard.prototype.computeScreenSpacePosition = function (scene, result) {
  ...
  const windowCoordinates = GWBillboard._computeScreenSpacePosition(
    modelMatrix,
    position,
    this._eyeOffset,
    scratchPixelOffset,
 ...
 * @param {GWBillboard} billboard The billboard to get the screen space bounding box for.
 ...
 * @private
 */
GWBillboard.getScreenSpaceBoundingBox = function (
  billboard,
  screenSpacePosition,
  result
) {
 ...
 *
 * @param {GWBillboard} other The billboard to compare for equality.
 * @returns {Boolean} <code>true</code> if the billboards are equal; otherwise, <code>false</code>.
 */
GWBillboard.prototype.equals = function (other) {
  return (
    this === other ||
    (defined(other) &&
      this._id === other._id &&
...
GWBillboard.prototype._destroy = function () {
